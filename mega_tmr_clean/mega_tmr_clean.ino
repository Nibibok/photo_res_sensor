...
constexpr float law = 1; // power of the inverse law of the photoresistors
uint8_t unSelect = 3; // which LRD is skipped (changes)
...
float tmr(int* pVals, const int cord){
  ...
  for (int i=0; i < REDUNDANCY; i++){
    ...
    // Loop over first reading
    if (i != unSelect) {
      if (pVals[i] < bitThreshold){
        noted += 1;
        }
      // Loop over second reading (always different)
...
void sunVec(float* r, float* result){
  ...
  // Apply the powerlaw aggreed on at start
  float sqSum = 0; 
  for (int i=0; i<3; i++){
    if (r[i] > 0){
      sqSum += pow(r[i], -2/law);    
  ...
  for (int i=0; i<3; i++)
    if (r[i] > 0){
      // The true zeros (ambiguous values) are not used in normalization 
...
void sunAng(float* v, float* result){
  ...
  // tangent ambiguity is not relevant since only 1 octant of r^3 is used
